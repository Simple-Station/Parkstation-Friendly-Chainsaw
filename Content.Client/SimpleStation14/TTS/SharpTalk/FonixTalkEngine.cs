...
namespace Content.Client.SimpleStation14.TTS.SharpTalk
{
    ...
    /// </summary>
    public sealed class FonixTalkEngine : IDisposable
    {
        #region Events
        /// <summary>
        ...
        */
        ...
        [DllImport("FonixTalk.dll")]
        static extern MMRESULT TextToSpeechAddBuffer(IntPtr handle, TtsBufferManaged.TTS_BUFFER_T buffer);
        ...
        /*
        [DllImport("FonixTalk.dll")]
        static extern MMRESULT TextToSpeechReturnBuffer(IntPtr handle, TtsBufferManaged.TTS_BUFFER_T buffer);
        */
        #endregion
        ...
        private void TtsCallback(int lParam1, int lParam2, uint drCallbackParameter, uint uiMsg)
        {
            ...
            else if (uiMsg == UiBufferMsg && _bufferStream != null && _buffer != null)
            {
                _bufferStream.Write(_buffer.GetBufferBytes(), 0, (int)_buffer.Length);
                var full = _buffer.Full;
                _buffer.Reset();
                Check(TextToSpeechAddBuffer(_handle, _buffer.Value));
            }
